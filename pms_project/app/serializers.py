...
from rest_framework.fields import CharField, FloatField, DecimalField, DateTimeField


from app.models import Supplier, Item, ItemCategory, Purchase, Quality, Tapeline, TapeType, TapeStock, Inventory, \
    Customer, Stereo, BagType, Order, Loom, LoomStock, Printing, PrintingStock ,Dispatch, RpmReading


class RpmReadingSerializer(ModelSerializer):

    circumference = DecimalField(source="loom.circumference",read_only=True, decimal_places=3, max_digits=12)
    ...
    class Meta:
        model = RpmReading
        fields = ("total", "id", "machine_no", "rpm", "time", "meters", "state", "loom", "circumference","quality", "denier", "color", "width", "length", "frame", "color", "weight", "quantity", "date_time",)
...
class LoomSerializer(ModelSerializer):
    ...
    class Meta:
        model = Loom
        fields = ("id", "loom_number", "circumference", "tape_type","bag_type", "quantity" ,"quality","denier", "date_time", )
